...
let TicketsService = class TicketsService {
    ...
    async scanTicket(qrToken) {
        ...
        if (ticket.seat.showing.status === enums_1.ShowingStatus.CANCELLED) {
        ...
        if (!updated) {
            throw new common_1.BadRequestException('No se pudo validar el ticket');
        }
        return { ticket: updated, alreadyCheckedIn: false };
    }
    async generateQrPng(qrToken) {
        const ticket = await this.prisma.ticket.findUnique({
            where: { qrToken },
            select: { id: true },
        });
