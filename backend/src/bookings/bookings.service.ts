...
import { BookingStatus, PurchaseChannel, Role, SeatStatus, ShowingStatus } from '../common/enums';
...
export class BookingsService {
  ...
  async createCustomerBooking(dto: CreateBookingDto, customerId: number) {
    ...
    if (reserveOnly && dto.useCredits === true) {
      throw new BadRequestException('No se puede usar credito en una reserva sin pago');
    }
    const status = reserveOnly ? BookingStatus.RESERVED : BookingStatus.PAID;
    const seatStatus = reserveOnly ? SeatStatus.HELD : SeatStatus.SOLD_ONLINE;
    ...
    const booking = await this.prisma.$transaction(async (tx) => {
      ...
      if (updated.count !== dto.seatIds.length) {
        throw new BadRequestException('No fue posible reservar los asientos');
      }
    ...
    if (!booking) {
      throw new BadRequestException('No fue posible registrar la reserva');
    }
    ...
    this.seatUpdatesGateway.notifySeats(
      dto.showingId,
      dto.seatIds.map((id) => ({ id, status: seatStatus })),
    );

    return booking;
  }

  async createPhysicalBooking(dto: CreatePhysicalBookingDto, employeeId: number, employeeRole: Role) {
    const normalizedEmployeeId = Number(employeeId);
    if (!Number.isInteger(normalizedEmployeeId) || normalizedEmployeeId <= 0) {
      throw new BadRequestException('Empleado invalido');
    }
    if (![Role.EMPLOYEE, Role.ADMIN].includes(employeeRole)) {
      throw new BadRequestException('Sin permisos para venta fisica');
    }
    const showing = await this.prisma.showing.findUnique({
      where: { id: dto.showingId },
      include: { movie: true },
    ...
    const booking = await this.prisma.$transaction(async (tx) => {
      const booking = await tx.booking.create({
        data: {
          ...
          channel: PurchaseChannel.PHYSICAL,
      ...
      const updated = await tx.seat.updateMany({
        where: { id: { in: dto.seatIds }, status: SeatStatus.AVAILABLE },
        data: { status: SeatStatus.SOLD_PHYSICAL },
      });
      if (updated.count !== dto.seatIds.length) {
        throw new BadRequestException('No fue posible registrar la venta');
      }
    ...
    this.seatUpdatesGateway.notifySeats(
      dto.showingId,
      dto.seatIds.map((id) => ({ id, status: SeatStatus.SOLD_PHYSICAL })),
    );

    return booking;
  }

  async listCustomerBookings(customerId: number) {
    const normalizedCustomerId = Number(customerId);
    if (!Number.isInteger(normalizedCustomerId) || normalizedCustomerId <= 0) {
      throw new BadRequestException('Cliente invalido');
    }
  ...
  private async sendBookingEmail({
    ...
    booking: {
      ...
      tickets?: Array<{ qrToken: string; seat?: { rowLabel: string; number: number } }>;
    };
    showing: { startTime: Date; movie: { title: string; format: string; language: string } };
    seats: Array<{ rowLabel: string; number: number }>;
    reserveOnly: boolean;
  }) {
    if (!email) {
      return;
    ...
    try {
      ...
      const sortedSeats = [...seats].sort(
        (a, b) => a.rowLabel.localeCompare(b.rowLabel) || a.number - b.number,
      );
